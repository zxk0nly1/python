...
class Douban:
    ...
    def save_mysql(self,content):#保存数据函数
        ...
        cursor=conn.cursor()
        sql_insert="""
        insert into movie_info(title,actors,regions,release_time,score,rank,types) values(%s,%s,%s,%s,%s,%s,%s)
        """
        cursor.executemany(sql_insert,t_list)
        conn.commit()
        print("数据存储成功")
